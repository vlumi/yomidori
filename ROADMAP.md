...
- [ ] **The first field round (2026-09-21), what remains of it.** Tap to focus on the camera; kanji as words with their readings, meanings and components from KANJIDIC2 and KRADFILE, the compounds they appear in, and every word read the same way with its pitch at a glance (はし); the still kept with a card only on request and removable later, the sentence editable; the meaning always asked, and a pitch question that picks the pattern from all the reading allows.
