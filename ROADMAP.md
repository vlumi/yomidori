...
- [ ] **Two pages, one sentence.** "Continues on next page" in the capture flow, for two shutters and for two screenshots alike: a sentence left open waits for the next still, shared or picked, and its continuation is tapped there; the fragments are joined and re-tokenized, both crops kept. *Done, as attaching pages: on a frozen page, "Add next page" takes the next still and joins its text to this one at the seam with no break, so a word cut by the page turn (樹 at the foot, 皮 at the head) tokenizes whole and a sentence runs on; a kept sentence runs from full stop to full stop across the pages' wrapped lines and carries every page's still. The textual half of the stitching idea below.*
...
- [ ] **Review on a keyboard.** Space to reveal, two keys to grade; and a *typed answer* mode, the reading typed in kana and checked strictly, which makes every review a few words of kana touch-typing on vocabulary actually met. An iPad with a keyboard gets the same. A typing tutor proper, with drills and speed, is a different product and stays out.
