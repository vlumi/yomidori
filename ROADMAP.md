...
- [ ] **Two pages, one sentence.** "Continues on next page" in the capture flow, for two shutters and for two screenshots alike: a sentence left open waits for the next still, shared or picked, and its continuation is tapped there; the fragments are joined and re-tokenized, both crops kept. *Done in first form: a kept sentence now runs from full stop to full stop across the page's wrapped lines; when it runs off the page, "Continues on next page" holds it, and the next still offers the fragment joined with its own beginning up to the first full stop, kept with both stills. Still to do: choosing where the continuation ends when the first full stop is not it.*
