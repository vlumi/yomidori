...
- [ ] **Positions on a vertical page.** *Answered on a paperback (2026-09-21): Live Text reads the vertical Mincho columns and its own selection is the tap; Vision's text request reads nothing vertical and only earns line positions on horizontal print.* Open: whether iOS 26's `RecognizeDocumentsRequest`, which Vision mode now tries first, reads vertical Japanese with boxes; a look at Vision mode on a novel page decides it. Boxes: it replaces the old request and the sentence crop, an own tap-to-token highlight and a furigana filter by height follow on vertical pages. No boxes: Vision leaves the picker and the route to positions is manga-ocr over the whole page, with a column finder (a projection profile) cutting the page first, after which Close-up goes too since it is never used.
- [ ] **manga-ocr over the whole page.** *Built in first form as Close-up: manga-ocr converted to Core ML (encoder as it is, decoder re-expressed as a cache-free step; ~210 MB at half precision, MIT), reading a window of about eight characters around a tap. Measured on rendered lines: 12 of 13 short windows exact, Mincho, vertical and blurred alike; whole long lines squeezed into its 224 pixels fail, hence the window. Optional at build (`make models`).* Wanted instead: the page cut into lines and every line read, so the engine gives a transcript with positions to compare with Live Text and to build the crop and the highlight on. On horizontal print Vision's boxes cut the page; on vertical print a column finder from the pixels is the work, a day or so, and speed on the phone (thirty to forty lines a page) the unknown.
...
- [ ] **Tap a word.** *Done as Live Text's selection: the selected run is read as words, each a row with reading, pitch, Keep and the meaning under it, its line the sentence.* Still to do: the app's own tap-to-token highlight, which needs positions (see the spike), and the recognized characters editable on the page as they are on the card.
