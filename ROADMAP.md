...
- [ ] **One card per word.** Dictionary form as the key; the sentence from the page as the front, OCR text plus the crop; reading, pitch and the collapsed meaning on the back; book and page recorded. A second sighting adds a sentence, never a card. *The model and the store are in Core (a JSON document in Application Support, tested); what remains is the Keep button on the tapped word, the still saved beside it, and the cards screen.*
