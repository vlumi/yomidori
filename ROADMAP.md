...
- [ ] **Freeze the page.** Camera view that only frames; shutter on screen (done), on the volume buttons and on the Camera Control (done, iOS 17.2 and up, where the system hands capture apps those presses); the still is what everything works on, pinch to zoom (done, with macro up close on phones that have it). *What remains: the shutter on iOS 16 and 17.0 stays on screen only, which is the API's floor, not a choice.*
- [ ] **Tap a word.** A tap anywhere on a token highlights the whole token, vertical or horizontal; a second tap on a neighbor extends over a compound. The reading appears in a bottom sheet in large kana, meaning collapsed under it. The recognized characters are editable in the sheet; the transcript copies out (done). The frozen still pinches to zoom and pans, so a small word is easy to hit (done). *First form on the page itself: a word selected on the still through Live Text's own selection is shown in the readout as if tapped in the strip, with its line as the sentence (iOS 17 and up; the strip stays the way on iOS 16); still to do is the app's own tap-to-token highlight, which needs positions Live Text does not give.*
- [ ] **Reach.** Everything touchable in the thumb's arc; the page can be swiped so the top of a column comes down.
- [ ] **The first field round (2026-09-21), what remains of it.** The drawer overlays the page instead of relaying it out, so dragging it neither flickers nor rezooms the still, and the still opens zoomed to the width with a one-finger way to zoom further; tap to focus on the camera; the selection highlight drifting off its word after a resize; kanji as words with their readings, meanings and components from KANJIDIC2 and KRADFILE, the compounds they appear in, and every word read the same way with its pitch at a glance (はし); the still kept with a card only on request and removable later, the sentence editable; the meaning always asked, and a pitch question that picks the pattern from all the reading allows.
...
- [ ] **Review.** FSRS with two grades; the queue is what is due when the app opens; no streaks, no reminders. Readings by default, meaning cards as an option per word. *Done in first form: FSRS-5 with its default parameters at 90 % retention, Again and Good, the queue from the Cards screen. Still to do: the meaning card as a per-word option.*
