...
**Version-bump policy (pre-1.0).** Bump `MARKETING_VERSION` only when a
...
2. **publish** — the interactive, stateful step. Prompts to bump
   ...
   highest tag. Stamps the changelog's *Unreleased (next build)* into
   `### build N` — and opens a `## vX.Y.Z` section for it when the version
   changed, leaving the previous heading over the builds it shipped. Commits on
   a `release/vX.Y.Z-N` branch, opens a PR, sets **auto-merge**, and **blocks
   until CI passes and it merges**. Red CI stops here — PR left open, nothing
   tagged or built.
...
`<prefix>` is `ios` (the scripts also know `mac`, unused); version is plain
SemVer; the suffix is the **build number**, not a beta/rc label:

```sh
ios/v0.1.0-1    # iOS, version 0.1.0, build 1
```
...
- **App Store Connect app record** for `fi.misaki.yomidori` — exists.
